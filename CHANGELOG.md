...
WROOM board with no PSRAM, and a large vendor frame map - turned up faults that
the hotspot setup had hidden. Before the fixes, with a small map and nothing
else changed, the dashboard stopped answering 25 seconds after boot and never
came back (`write(): fail on fd 50, errno: 11` once a second on the console),
while the recording ran on untouched.

**The web server**

- **The page went out in parts of up to 73 KB per socket write.** lwIP's send
  buffer is a few kilobytes; in station mode the write came back EAGAIN and the
  page died half-sent. It now goes out in 2 KB slices with a real
  Content-Length, pre-compressed (`src/webpage_gz.h`, 3.3x smaller, generated by
  `tools/gen_page_gz.py` and checked by `run_tests.sh`).
- **Nothing stopped a stalled send.** A client that stopped reading held the
  one-client-at-a-time server for up to ten seconds per slice. Every response
  now gives up after `WEB_SEND_MAX_MS`, a single write is bounded with
  `SO_SNDTIMEO`, and an abandoned page is closed with a reset instead of being
  retransmitted for minutes out of the heap new connections need.
- **`/api/status` and `/api/dash` rebuilt their reply with a reallocation per
  append**, five times a second - and then reserved 16 KB in one piece for a
  2 KB reply, which failed once the heap was fragmented. Static buffers, sized
  to what the replies measure.
- **Unknown `/api/` paths and `/favicon.ico` were answered with the whole
  page.** Now a small 404 and a 204.
- **`/api/signals` streamed a large map back to back without yielding**, so
  the reply was truncated and the page never finished starting. It yields
  between chunks now.

**Memory**

- **The heap gauge read the wrong pool.** `ESP.getFreeHeap()` counts 32-bit
  IRAM that `malloc()` can never return, so it reported ~41 KB that did not
  exist. `src/mem.cpp` reads the 8-bit pool, tracks the largest free block and
  its low-water mark, logs a boot ladder into the recording, and traps failed
  allocations - which is how the real mechanism was found: every refused
  allocation was the ~2.3 KB receive buffer an incoming connection needs.
- **`DBC_HEAP_RESERVE` had been tuned against that wrong gauge.** Against the
  real figure it exceeded the whole heap and every message on both buses was
  dropped. It is 40 KB now, and a zero budget is reported as a configuration
  error rather than as a map that did not fit.
...
- **The receive task wrote to the SD card.** The overflow and sticky-flag paths
  ...
  line; nothing in that task touches the card.

**Frame maps**

- **The DBC was read one byte at a time**: one filesystem call per byte, which
...
- **A readable map that did not fit was reported as thousands of unparsable
  ...
  the `NS_` keyword list no longer counts as an error in every file.
- **Signals of a dropped message were attached to the previous one.** When the
  message table was full, the `SG_` lines under the message that did not fit
  were appended to the last message that did - which then decoded its frames
  with another message's bits. They are skipped now, and a test fails if they
  are not.
- **A `VAL_` line naming a clipped signal in full never matched**, so its value
  table was lost. Names are compared the way they were stored.
- **The SD card could stay wedged after a USB flash.** A reset mid-write leaves
  the card holding its data line busy; the firmware now clocks it free before
  mounting and retries the whole speed ladder with longer pauses.

**Desk tools and tests**

- **`preview_dashboard.py` merged value tables across messages** that reuse a
  signal name (normal in J1939): wrong labels, and a heap estimate 2.3x the
  firmware's. Keyed by message and signal now; the parser comparison in
  `run_tests.sh` counts value labels and includes a file that exercises it.
- `run_tests.sh` runs from any directory; the page-versus-firmware field check
  accepts underscored names; the host shims answer heap questions with figures
  measured on a board instead of a flat 200 000.
